...
func ServeOAuthHandler(w http.ResponseWriter, r *http.Request) {
	...
	lang := template.GetLanguageFromRequest(r)
	tpl := template.Create("oauth", lang)
	...
	err = app.CacheRedis["outh"].Set("oauth:"+referenceKey, sessionData, 60*time.Minute)
	if err != nil {
	...
	pErr := encrypt.CheckPrivateKey(session.Private, session.PrivateHash)
