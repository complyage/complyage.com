...
//||------------------------------------------------------------------------------------------------||
      ...
      const MediaPreview: React.FC<MediaPreviewProps> = ({ media, onEdit }) => (
            <div className="flex flex-col w-full relative items-center justify-center mx-auto">
                  ...
                  <div className={`group w-full border-gray-400 rounded-2xl pt-3 pb-3 px-6 font-bold text-center flex flex-col items-center justify-center transition-colors bg-black/20`} style={{ minHeight: 300 }}>
                        <div className="min-h-42 overflow-hidden w-full flex flex-col items-center mb-2 justify-center rounded-xl">
                              {media?.blob ? (
                                    <>
                                          <img
                                                src={`data:${media.mime || "image/jpeg"};base64,${media.blob}`}
                                                alt="Preview"
                                                className="max-w-full h-auto w-full rounded-xl"
                                          />
                                    </>
                              ) : (
                                    <>
                        ...
                        {onEdit && (
                              <button
                                    type="button"
                                    onClick={onEdit}
                                    className="btn btn-primary text-yellow-400"
                              >
                                    Edit
                              </button>
                        )}
