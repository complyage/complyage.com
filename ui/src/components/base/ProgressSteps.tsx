...
const ProgressSteps: React.FC<ProgressStepsProps> = ({steps, currentStep, className}) => {
	...
	const activeStep = steps[stepNow - 1];

	return (
		<div className={`w-full ${className ?? ""}`}>
			{/* Current step description (only once) */}
			{activeStep?.description ? <h2 className="text-xl leading-snug text-gray-200 font-bold border-b border-b-gray-300 py-2 mb-4">{activeStep.description}</h2> : null}

			{/* Stepper */}
			<div className="flex items-center w-full" role="list" aria-label="Progress">
				{steps.map((s, i) => {
					const stepNum = i + 1;
					const isCompleted = stepNum < stepNow;
					const isActive = stepNum === stepNow;
