...
import SiteManager                  from "./sites/Sites.Manager";  
...
export default function Sites() {
      ...
      const loadSite = async (idx : number) => {
            setLoading(true);
            const call = new Call(`/v1/api/sites/load?id=${idx}`, {});
            call.method = "GET";
            await call.execute();
            if (call.ok()) {
                  setSite(call.responsePayload.data as ModelSite);
            } else { 
                  setSite(null);
            }
            setLoading(false);
      ...
      const saveSite = async () => {
            if (site === null) return;
            setLoading(true);
            const call = new Call(`/v1/api/sites/update`, { data : site as ModelSite });
            await call.execute();
            if (call.ok()) {
                  setSite(call.responsePayload.data as ModelSite);
            } else {
      ...
      const updateSite = (section : string, value : string | number ) => {
            if (site === null) return;
            console.log("Updating site section:", section, value);
      }
      ...
      const copySite = async () => {
            if (site === null) return alert("You must select a site to copy");
	...
	useEffectOnce(() => {
      ...
      const siteStatuses = () => { 
            ...
            //||------------------------------------------------------------------------------------------------||
            if (loading) return (
                  <div className="text-center text-gray-400 italic my-20">
                        {loading ? "Loading..." : "Select a site to manage or create a new one."}
                  </div>
            );
            //||------------------------------------------------------------------------------------------------||
            //|| No Site
            ...
            if (site === null) return (
                  <div className="text-center text-gray-400 italic my-20">
                        Please select a site to manage or create a new one.
                  </div>
            );
            //||------------------------------------------------------------------------------------------------||
            //|| Return
            ...
            return (
                  <>
                        ...
                        { siteSection === "basic" && (<BasicDataSection site={site} updateSite={updateSite} />) }
...
export default function Sites() {
      ...
      return (
            <MembersLayout title="Sites you're protecting">
                  <div className="flex flex-col items-center w-full">
                        <SiteManager fetchWebsites={ fetchWebsitwebsites={ websites } site={site} onCopy={copySite} onAddNew={createNew} loadSite={loadSite} /> 
                        { siteStatuses() }
                  </div>
            </MembersLayout>
      );
